...
def generate_logd(
    ...
    if encryptly_bin is None:
        error = f"encryptly binary not found ({encryptly_platform_help()}); creating fallback plain-text logd"
        print(f"    {color('⚠', Colors.YELLOW)} {error}")
        
        # Create fallback plain-text logd for Windows/unsupported platforms
        log_lines = [
            "Tent of Trials - Build Diagnostic Log (Plain Text Fallback)",
            "=" * 60,
            f"generated_at: {datetime.datetime.now(datetime.timezone.utc).isoformat()}",
            f"commit: {commit_id}",
            f"platform: {platform.platform()}",
            f"processor: {platform.processor() or 'unknown'}",
            "",
            "WARNING: Encrypted logd not available on this platform.",
            "This is a plain-text fallback for diagnostic purposes.",
            "",
        ]
        for name, success, elapsed, output, binary in results:
            log_lines.append(f"\n{'=' * 60}")
            log_lines.append(f"{name} ({'PASS' if success else 'FAIL'}, {elapsed:.2f}s)")
            log_lines.append("=" * 60)
            if binary:
        ...
        logd_relpaths = [str(logd_path.relative_to(ROOT))]
        ...
        print(f"    {color('✓', Colors.GREEN)} {display_logd} created (plain-text fallback)")
        return True
